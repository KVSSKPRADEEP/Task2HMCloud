...
resource "aws_efs_file_system" "myefs" {
  ...
  tags = {
    Name = "Mywebsitefilesystem"
  }
}
 
resource "aws_efs_mount_target" "alpha" {
  file_system_id = aws_efs_file_system.myefs.id
  subnet_id      = aws_subnet.alpha1.id
}
...
provisioner "remote-exec" {
      inline = [ 
        # sudo su << \"EOF\" \n echo \"<img src='${aws_cloudfront_distribution.my_cd.domain_name}'>\" >> /var/www/html/index.html \n \"EOF\"
            "sudo su << EOF",
         "echo \"<img src='https://${aws_cloudfront_distribution.my_cd.domain_name}/${aws_s3_bucket_object.pradeeps3301bucket.key}'>\" >> /var/www/html/index.html",
          "EOF"
     ]
  }
}
